...
$parentModule = 'DomainManagement'
if (-not $ModuleName) { $ModuleName = 'DomainManagement.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\DomainManagement'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\DomainManagement.psd1"
$prereqHash = @{
