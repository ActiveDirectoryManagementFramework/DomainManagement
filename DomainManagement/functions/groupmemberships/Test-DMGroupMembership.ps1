    ...
    process {
        :main foreach ($groupMembershipName in $script:groupMemberShips.Keys) {
            ...
            foreach ($assignment in $assignments) {
                ...
                New-TestResult @resultDefaults -Type Add -Identity "$(Resolve-String -Text $assignment.Assignment.Group)þ$($assignment.ADObject.ObjectClass)þ$(Resolve-String -Text $assignment.ADObject.SamAccountName)" -Configuration $assignment -ADObject $adObject
            }
